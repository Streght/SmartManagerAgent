...
public class SlotAccepted<K, T> extends Command<K, T, String> {

    private Activity<T> activity;
    private Serializable slot;

    public SlotAccepted(Message<String> message, Agent<K, T, String> agent) {
        super(message, agent);
    ...
    public boolean execute() {
        ...
        if (results != null && results.get(this.activity) != null) {
            ...
            if (res.finished()) {
                if (res.accepted()) {
                    ...
                    // Delete attribute waitConfirmation in state
                }
